    ...
    <div className="flex flex-col h-full bg-[#1c1c1c] text-white">
      ...
      <div className="p-3 bg-[#252525] border-b border-[#333] flex justify-between items-center">
          ...
          <button 
            ...
            >
                {config.enablePostProcessing ? '已开启' : '已关闭'}
            </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-5">
        ...
        {activeTab === 'Kernel' && (
           <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
             <div className="space-y-3">
                 ...
                 {config.ao && (
                    <>
                        <SliderControl label="强度 (Intensity)" value={config.aoIntensity} min={0} max={5} step={0.1} onChange={(v) => update('aoIntensity', v)} />
                        <SliderControl label="半径 (Radius)" value={config.aoRadius} min={0.1} max={2} step={0.1} onChange={(v) => update('aoRadius', v)} />
                    </>
                 )}
             </div>
           </div>
        ...
        {activeTab === 'Imager' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="space-y-3">
                    <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">相机成像 (Imager)</h3>
                    
                    <SliderControl label="曝光 (Exposure)" value={config.exposure} min={-2} max={2} step={0.05} onChange={(v) => update('exposure', v)} />
                    <SliderControl label="对比度 (Contrast)" value={config.contrast} min={-1} max={1} step={0.05} onChange={(v) => update('contrast', v)} />
                    <SliderControl label="饱和度 (Saturation)" value={config.saturation} min={-1} max={1} step={0.05} onChange={(v) => update('saturation', v)} />
                    <SliderControl label="亮度 (Brightness)" value={config.brightness} min={-1} max={1} step={0.05} onChange={(v) => update('brightness', v)} />
        ...
        {activeTab === 'Post' && (
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div>
                     ...
                     {config.bloom && (
                        <div className="space-y-3 pl-2 border-l border-[#333]">
                            <SliderControl label="强度 (Intensity)" value={config.bloomIntensity} min={0} max={5} step={0.1} onChange={(v) => update('bloomIntensity', v)} />
                            <SliderControl label="阈值 (Threshold)" value={config.bloomThreshold} min={0} max={2} step={0.05} onChange={(v) => update('bloomThreshold', v)} />
                            <SliderControl label="半径 (Radius)" value={config.bloomRadius} min={0} max={1.5} step={0.05} onChange={(v) => update('bloomRadius', v)} />
                        </div>
                     )}
                </div>
                ...
                <div>
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">镜头暗角 (Vignette)</h3>
                    ...
                    {config.vignette && (
                        <div className="space-y-3 pl-2 border-l border-[#333]">
                            <SliderControl label="深度 (Darkness)" value={config.vignetteDarkness} min={0} max={1} step={0.05} onChange={(v) => update('vignetteDarkness', v)} />
                            <SliderControl label="范围 (Offset)" value={config.vignetteOffset} min={0} max={1} step={0.05} onChange={(v) => update('vignetteOffset', v)} />
                        </div>
                    )}
                </div>
                ...
                <div>
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">光谱色散 (Spectral)</h3>
                    ...
                    {config.chromaticAberration && (
                         <div className="space-y-3 pl-2 border-l border-[#333]">
                             <SliderControl label="色散强度 (Dispersion)" value={config.chromaticAberrationOffset} min={0} max={0.05} step={0.001} onChange={(v) => update('chromaticAberrationOffset', v)} />
                         </div>
                    )}
                </div>
                ...
                {/* Noise */}
                 <div>
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">胶片颗粒 (Film Grain)</h3>
                    ...
                    {config.noise && (
                        <div className="space-y-3 pl-2 border-l border-[#333]">
                            <SliderControl label="颗粒度 (Amount)" value={config.noiseOpacity} min={0} max={0.5} step={0.01} onChange={(v) => update('noiseOpacity', v)} />
                        </div>
                    )}
                </div>
